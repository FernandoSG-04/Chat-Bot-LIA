...
async function handleSuccessfulLogin(username, remember) {
  ...
  try {
    ...
    const isLocal =
      ['localhost', '127.0.0.1', ''].includes(hn) || !hn.includes('.');
    if (isLocal) {
    ...
    if (err && err.status) {
      if (err.status === 401) {
        detail =
          'No autorizado para emitir sesión. Verifica la clave de acceso (X-API-Key) o vuelve a iniciar sesión. Código: 401';
      } else if (err.status === 429) {
        detail =
          'Demasiadas solicitudes. Espera un momento y vuelve a intentarlo. Código: 429';
      } else if (err.status === 400) {
        detail =
          'Usuario inválido para emitir sesión. Revisa el nombre de usuario. Código: 400';
      } else if (err.status >= 500) {
        detail =
          'Servidor no disponible en este momento. Inténtalo más tarde. Código: ' +
          err.status;
      }
    }
    showError(detail);
...
async function handleFailedLogin() {
  ...
  if (loginState.attempts >= LOGIN_CONFIG.maxAttempts) {
    await lockUser();
    showError(
      `Demasiados intentos fallidos. Cuenta bloqueada por ${LOGIN_CONFIG.lockoutDuration / 60000} minutos.`
    );
  } else {
...
function validateUsername(username = null) {
  ...
  if (value.length > 0) {
    elements.username.style.borderColor = isValid
      ? 'var(--success)'
      : 'var(--error)';
  } else {
...
function validatePassword(password = null) {
  ...
  if (value.length > 0) {
    elements.password.style.borderColor = isValid
      ? 'var(--success)'
      : 'var(--error)';
  } else {
